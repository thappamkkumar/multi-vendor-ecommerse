...
<section class="p-0 pt-2 p-md-4 h-auto parent_add_container" >
	...
	<!-- using Modal for Seller/Vendor  list-->
	<div class="modal fade" id="vendorModal" tabindex="-1" aria-labelledby="vendorModalLabel" aria-hidden="true">
	  <div class="modal-dialog modal-dialog-scrollable">
		<div class="modal-content">
		  <div class="modal-header">
			<h5 class="modal-title" id="categoryModalLabel">Select Seller</h5>
			<button type="button" class="btn btn-light" data-bs-dismiss="modal" aria-label="Close">
				<i class="bi bi-x-lg  fw-bolder fs-5 "></i>
			</button>
		  </div>
		  <div class="modal-body">
			<ul class="list-group">
				@foreach($users as $user)
					<li class="list-group-item p-0">
						<button type="button" class="btn  w-100 d-block text-start btn-light rounded-0  "
							onclick='window.location.href =  `{{route("admin.productList", ["filterType" => "seller", "filterName" => $user->id])}}`;'>
							{{ $user->vendors->name }} 
						</button>
					</li>	
	...
	<div class="w-100 h-auto p-1 p-md-4 shadow rounded">
		...
		<div class="w-100 d-flex flex-wrap justify-content-between align-items-center ">
			...
			<div class="">
				...
				<form action="{{route('admin.productSearch')}}" method="POST">
					@csrf
		...
		<div class="py-2 px-2 d-flex flex-wrap justify-content-between align-items-center  table_header " style="--bs-bg-opacity:0.3;">
			...
			<div class="py-2">
				<!-- button seller list-->
				<button type="button" class="btn " data-bs-toggle="modal" data-bs-target="#vendorModal"id="userCustomerBtn">
				  <i class="bi bi-filter  pe-1  "></i>Seller
				</button>
			...
			<tbody>
				...
				@foreach($products as $product)
					...
					<tr class="text-danger">
						...
						<td class=" @if($product->is_active == 0)text-danger @endif ">{{$product->user->vendors->name}}</td>
						...
						<td class="col-1  ">
							...
							onclick='window.location.href =  `{{route("admin.productDetail", ["product" => $product])}}`;'>
