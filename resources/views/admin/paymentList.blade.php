			...
			<div class="">
				...
				<form action="{{route('admin.paymentSearch')}}" method="POST">
					...
					<div class="input-group-text flex-wrap  align-items-center   bg-transparent border-0">
						<input type="search" name="userSearchInput" placeholder="Enter user" 
			...
			<tbody>
				...
				@foreach($payments as $payment)
					<tr>
						...
						<td class="col-1">
							<a href="{{ route('admin.userProfile', ['user' => $payment->user]) }}" class="   text-decoration-none">
									{{ $payment->user->vendors->name }}
							</a>
						</td>
						<td class="col-1">
							<a href="{{ route('admin.orderDetail', ['order' => $payment->order]) }}" class="text-decoration-none">
									{{ $payment->order->order_number }}
							</a>
						</td>

						<td class="col-1  ">{{$payment->amount}}</td>  
						<td class="col-1  ">{{ $payment->created_at->diffForHumans()}}</td>  
